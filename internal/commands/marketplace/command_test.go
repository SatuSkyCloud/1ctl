...
// TestFlagsHaveDestination ensures every Required flag in the marketplace
	...
	walkCommands(Command(), func(cmd *cli.Command) {
		for _, f := range cmd.Flags {
			...
			if hasNilDestination(f) {
				t.Errorf("command %q: required flag %q has no Destination — value will be lost", cmd.Name, flagDisplayName(f))
			}
		}
	})
}
...
func flagDisplayName(f cli.Flag) string {
